...
const OperationLogsModal = ({ open, onClose }) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'result',
      width: 85,
      render: (v) => (v === 'error' ? <Badge status="error" text="失败" /> : <Badge status="success" text="成功" />),
    },
    {
      title: '备注',
      dataIndex: 'detail',
  ...
  return (
    ...
    >
      <Space wrap style={{ marginBottom: 16 }}>
        ...
        />
        <RangePicker showTime value={filters.range} onChange={(dates) => updateFilter({ range: dates || null })} />
        <Button
          icon={<ClearOutlined />}
          onClick={() =>
            updateFilter({
