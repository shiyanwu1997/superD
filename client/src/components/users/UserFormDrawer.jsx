...
const UserFormDrawer = ({
  ...
  const handleDrawerOpen = (open) => {
    if (open && editUser) {
      getUserProgramPermissions(editUser.id)
        .then((p) => setProgramPerms(p || []))
        .catch(() => {});
    }
      ...
      if (editUser) {
        ...
        if (res.success) {
          ...
          if (onSwitchToEdit) {
            const newUser = {
              ...res.user,
              projectPermissions: (values.projectIds || []).map((pid) => ({ projectId: pid })),
            };
    ...
    >
      <Form form={form} layout="vertical" onFinish={handleSubmit}>
        ...
        <Form.Item name="roleId" label="角色" rules={[{ required: true, message: '请选择角色' }]}>
          <Select
            // 仅在编辑内置 admin 账户时锁定角色；普通管理员（roleId=2）可为自己创建的用户选"普通用户"
          ...
          >
            ...
            {Number(user?.roleId) === 2 && <Select.Option value={3}>普通用户</Select.Option>}
          </Select>
        </Form.Item>
        ...
        <div style={{ marginTop: 8, padding: '12px 0', borderTop: '1px solid #f0f0f0' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 12 }}>
            {editUser && programPerms.length === 0 && pendingProgramPerms.length === 0 && (
              <span style={{ color: '#9ca3af', fontSize: 12 }}>未限制</span>
            )}
            {!editUser && pendingProgramPerms.length === 0 && (
              <span style={{ color: '#9ca3af', fontSize: 12 }}>未限制</span>
            )}
            {/* 编辑模式：API 加载的程序权限 */}
            {editUser &&
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <Select
              ...
              placeholder="选择机器"
              value={progMachine}
              onChange={handleProgMachineChange}
              allowClear
              options={projects
                .filter((p) => {
            ...
            <Select
              style={{ minWidth: 220 }}
              mode="multiple"
              placeholder="选择程序（可多选）"
              value={progSelected}
              onChange={setProgSelected}
              loading={loadingProgs}
              disabled={!progMachine}
              options={progList
                .filter((p) => {
            ...
            />
            <Button type="primary" disabled={!progMachine || progSelected.length === 0} onClick={handleAddProgramPerms}>
              添加
            </Button>
          </div>
        </div>
